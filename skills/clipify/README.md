...
A [Claude Code](https://claude.com/claude-code) skill that turns **your** long videos into social-ready clips.

Point it at a video file on your machine and it will:
...
Most "auto-clip" tools are either expensive SaaS, slow, or produce slop. This skill is built for talking-head dialogue (interviews, podcasts, two-person setups) — paste a path to your recording and cut LinkedIn / TikTok / Reels / Shorts from it.
...
This skill lives in this project at `skills/clipify/`. Open the Thaisvideoediting-Claude folder in Cursor, or symlink it for Claude Code:

```bash
ln -sfn "$(pwd)/skills/clipify" ~/.claude/skills/clipify
```
...
│   └── detect_transients.py  # impact sounds for action-mode clips
└── README.md
```
